...
* [Синхронизация потоков и координация доступа к разделяемым ресурсам](#%D1%81%D0%B8%D0%BD%D1%85%D1%80%D0%BE%D0%BD%D0%B8%D0%B7%D0%B0%D1%86%D0%B8%D1%8F-%D0%BF%D0%BE%D1%82%D0%BE%D0%BA%D0%BE%D0%B2-%D0%B8-%D0%BA%D0%BE%D0%BE%D1%80%D0%B4%D0%B8%D0%BD%D0%B0%D1%86%D0%B8%D1%8F-%D0%B4%D0%BE%D1%81%D1%82%D1%83%D0%BF%D0%B0-%D0%BA-%D1%80%D0%B0%D0%B7%D0%B4%D0%B5%D0%BB%D1%8F%D0%B5%D0%BC%D1%8B%D0%BC-%D1%80%D0%B5%D1%81%D1%83%D1%80%D1%81%D0%B0%D0%BC)
    * [Примитивы синхронизации](#%D0%BF%D1%80%D0%B8%D0%BC%D0%B8%D1%82%D0%B8%D0%B2%D1%8B-%D1%81%D0%B8%D0%BD%D1%85%D1%80%D0%BE%D0%BD%D0%B8%D0%B7%D0%B0%D1%86%D0%B8%D0%B8)
        ...
        * [Синхронизация потоков: java.util.concurrent](#%D1%81%D0%B8%D0%BD%D1%85%D1%80%D0%BE%D0%BD%D0%B8%D0%B7%D0%B0%D1%86%D0%B8%D1%8F-%D0%BF%D0%BE%D1%82%D0%BE%D0%BA%D0%BE%D0%B2-javautilconcurrent)
            ...
            * [Phaser](#phaser)
* [Атомарные переменные](#%D0%B0%D1%82%D0%BE%D0%BC%D0%B0%D1%80%D0%BD%D1%8B%D0%B5-%D0%BF%D0%B5%D1%80%D0%B5%D0%BC%D0%B5%D0%BD%D0%BD%D1%8B%D0%B5)
* [Замки(Locks)](#locks)
...
Phaser похож на CyclicBarrier, но позволяет изменять кол-во участников в runtime.
Задачи могут регистрироваться методом `register()` в любое время, а также (необязательно) прекращать
регистрацию по прибытию к барьеру методом `arriveAndDeregister()`, либо ожидать на барьере как и в CyclicBarrier
с помощью метода `arriveAndAwaitAdvance()`. Phaser поддерживает номер фазы, который инкрементируется при прохождения
барьера всеми участниками. Метод `awaitAdvance(int)` позволяет ждать определенной фазы, не задерживаясь на остальных. 

[Пример использования Phaser](src/main/java/learn/jconcurrency/synchronizators/PhaserExample.java)
...
Атомарные переменные лежат в основе многих неблокирующих алгоритмов и структур таких как ConcurrentHashMap
CopyOnWriteArrayList.  
Существуют проблемы атомарного доступа к данным. Например операция инкремента имеет неатомарный характер, 
так как она состоит из чтения, увеличения и записи. Это значит, что мы можем в разных потоках увидеть промежуточный 
результат вычисления инкремента. Более того, если один поток выполняет операцию инкремента, то другой поток может
не увидеть эти изменения или увидеть не полностью.

Атомарные переменные предоставляют CAS (Compare And Swap) операции. Перед тем как произвести операцию мы можем для
начала сравнить существующее значение с тем значением, которое мы ожидаем увидеть, и если это значения равны,
мы можем произвести запись в ячейку памяти или выполнить арифметическую операцию с записью результата.
Такое действие мы выполняем в цикле до тех пор пока ожидаемое значение не равно существующему значению.
Это называется Оптимистичной Блокировкой.


## Locks

Интерфейс `Lock` и класс `ReentrantLock` реализуют ту же самую идиому, для которой предназначены
другие синхронизаторы. Они обеспечивают доступ к критической секции и регламентируют следующую семантику:
всякий поток, который хочет получить доступ к критической секции должен вызвать метод `lock()`, `tryLock()`, 
или `lockInterruptibly()` то есть запирание замка, перед выполнением действий на целевом объекте, 
а по завершении работы поток должен разблокировать замок методом `unlock()`.   
`ReentrantLock` используется в ConcurrentHashMap на уровне бакета, чтобы уменьшить гранулярность блокировки.  
Интерфейс `ReadWriteLock` и класс `ReentrantReadWriteLock` поддерживают пару замков, один для операций
read-only, другой для записи. Замок для чтения позволяет работать нескольким читающим потокам, если нет
замка для записи, который является эксклюзивным.  
Lock позволяет реализовать более легковесный вариант синхронизации по сравнению с synchronized.
